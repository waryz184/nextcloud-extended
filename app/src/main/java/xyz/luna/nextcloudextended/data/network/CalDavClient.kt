...
class CalDavClient(
    ...
    // Fetches all calendars in one PROPFIND and returns event calendars + task lists separately
    fun getAllCalendarData(
        onSuccess: (eventCalendars: List<Pair<String, String>>, taskLists: List<Pair<String, String>>) -> Unit,
    ...
    <c:supported-calendar-component-set />
        ...
        client.newCall(request).enqueueTracked(object : okhttp3.Callback {
            ...
            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                ...
                try {
                    val eventCals = parseCalendars(body, "VEVENT")
                    val taskLists = parseCalendars(body, "VTODO")
...
<c:calendar-query xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  ...
  <c:filter>
    ...
    fun getNotes(onSuccess: (List<NextcloudNote>) -> Unit, onFailure: (Exception) -> Unit) {
        val url = "$baseUrl/index.php/apps/notes/api/v1/notes"
    ...
    fun updateNote(noteId: Int, title: String, content: String, category: String, favorite: Boolean, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        val url = "$baseUrl/index.php/apps/notes/api/v1/notes/$noteId"
    ...
    // Parse calendars PROPFIND response, filtering by supported component type ("VEVENT" or "VTODO")
    private fun parseCalendars(xml: String, filterComponent: String): List<Pair<String, String>> {
        ...
        for (resp in responseRegex.findAll(xml)) {
            ...
            if (!calendarResourceTypeRegex.containsMatchIn(respStr)) continue
    ...
    private fun unescapeXml(xml: String): String {
        var result = xml
            ...
            .replace("&quot;", "\"")
            .replace("&apos;", "'")
        result = Regex("&#([0-9]+);").replace(result) { it.groupValues[1].toInt().toChar().toString() }
        result = Regex("&#x([0-9a-fA-F]+);").replace(result) { it.groupValues[1].toInt(16).toChar().toString() }
        return result
    }
    ...
    private fun parseEventsFromReport(xml: String): List<CalendarEvent> {
        ...
        for (m in calDataRegex.findAll(xml)) {
            events.addAll(parseIcs(unescapeXml(m.groupValues[1])))
        }
        return events
    }
...
<d:propfind xmlns:d="DAV:">
  <d:prop>
    ...
    private fun parseFiles(xml: String, requestPath: String): List<NextcloudFile> {
        ...
        for (resp in responseRegex.findAll(xml)) {
            ...
            val displayName = displaynameRegex.find(respStr)?.groupValues?.get(1) ?: run {
                ...
                val last = parts.lastOrNull { it.isNotEmpty() } ?: "Sans nom"
                last
            }
    ...
    fun createFolder(parentHref: String, folderName: String, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        ...
        val url = "$baseUrl$cleanParent$folderName/"
    ...
    fun uploadFile(parentHref: String, fileName: String, fileBytes: ByteArray, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        val cleanParent = if (parentHref.endsWith("/")) parentHref else "$parentHref/"
        val url = "$baseUrl$cleanParent$fileName"
